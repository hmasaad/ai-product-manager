...
Decision Versioning keeps the complete product decision history. A change writes DEC-142 v2. DEC-142 v1 stays on the ledger. The family names the current version, the superseded versions, and the stated diff between them: Online reports → Support queued offline generation, with reasons Offline usage increased, New synchronization infrastructure, and Customer feedback when those lines are stated. HarborLine, clinic, and Offline reports #142 stay on v1 unless a later version is named.

A recorded decision is re-evaluated when a trigger fires. The pipeline retrieves the original decision, supporting evidence, and assumptions, collects new evidence, compares old vs new state, names the changed assumption, calculates impact, and writes a proposal card: original decision, trigger, changed assumption, impact, proposed action, and confidence. The primary screen is What Changed?: stated before and after (8% → 31%, 4/month → 27/month, No enterprise requirement → Required when those lines are stated), the affected assumption, and Review Decision. It is not a long model writeup. Re-evaluation priority is Evidence Change × Decision Impact × Confidence × Business Exposure. Each factor is a 0–1 signal with a reason. Stated 0.90 × 0.80 × 0.86 × 0.75 is 0.46, classified High against fixed thresholds (Low below 0.20, Medium below 0.40, High below 0.70, Critical at or above 0.70). Unnamed factors leave the product unnamed. The engine leaves the recorded choice in place. A person still owns Keep Decision, Review, or Change Decision. The decision moves through states: ACTIVE, TRIGGERED, UNDER_REVIEW, then VALIDATED, DECISION_CHANGED, or DECISION_RETAINED. A change writes a new version of the same decision, such as DEC-142 v2 after DEC-142 v1. The old version stays. Together the versions are the product decision history. Stated later evidence such as a 38% rise in offline usage stays stated. A greenfield brief with no new evidence stays quiet.

Those pieces now share one path. Product Discovery feeds opportunities. The Decision Engine writes the Decision Ledger. Re-evaluation asks Architect, Analytics, and Feedback, then runs impact analysis. A person still approves. An approved change becomes the updated decision. The six specialists still run; their findings hand to the next hop on that path.

The Product Knowledge Graph connects customer, problem, opportunity, feature, and requirement, then the decision, risk, experiment, and metric that hang off that requirement, and the outcome they aim for. Decision lineage walks the same product: customer feedback to evidence, assumption, decision, feature, engineering, and product outcome, then back to new evidence. The PM can ask which features sit on the biggest customer problems, which roadmap items have weak evidence, which requirements hang on unvalidated assumptions, which decisions new customer feedback moves, and what caused a feature to exist.
...
The Autonomous Product Loop is the operating cycle: observe, understand, discover, analyze, propose, validate, decide, plan, execute, measure, learn, then observe again. Observe through propose can run from evidence. Validate, decide, and execute wait on a person. A live signal stays on analyze until the investigation is signed complete. A problem brief stays on validate. Mandatory gates hold decide. After measure and learn, the loop observes again. When a recorded decision is challenged, a tighter cycle runs: observe, detect, re-evaluate, propose, approve, execute, measure, learn, then observe again.

Phase 3 is the autonomous path those pieces now share: product data → continuous monitoring → decision re-evaluation → product opportunity → new PRD → AI Architect → engineering. A person still signs before a commit. The PM is an operating system, not a document printer.
